...
{
	apex = posA + vel;				// вершина конуса
	float size = sizeA + sizeB;		// суммарный размер - он же радиус касаемой окружности. Её центр совпадает с центром B
	Pose::vec dir=posB-posA;		
	float angle=0;
	if(dir.length() < size)	
		 angle=M_PI/2;			// если объект слишком близко. Конус вырождается в полуплоскость
	else
	{
		//assert(dir.length_squared()-size*size);						// это на случай если мы уже внутри другого объекта. Решу потом что тогда делать
		float touchDistance=sqrt(dir.length_squared()-size*size);	// расстояние от центра A до точки касания. Из теоремы пифагора. 
		angle=acos(touchDistance/dir.length());				// это угол как его там конуса. Или половина его		
	}	
	dirLeft = normalise(dir).rotate(angle);						// получаем направление левой касательной
	dirRight = normalise(dir).rotate(-angle);					// получаем направление правой касательной
	flags = 0;
	if(apex.isZero())
		flags |= zeroApex;
	...
	distanceLeft = dirLeft[1] * apex[0] - dirLeft[0] * apex[1];	// расстояние до центра 
	distanceRight = dirRight[1] * apex[0] - dirRight[0] * apex[1];	// расстояние до центра 
}

// рассчитываем сегмент окружнсти , отсекаемый VelocityObstacle
...
{
	float normProj = (dirLeft[1] * ray[0] - dirLeft[0] * ray[1]);			// длина проекции луча на нормаль
...
{
	float normProj = (dirRight[1] * ray[0] - dirRight[0] * ray[1]);			// длина проекции луча на нормаль
	// if ray and edge are parallel
	if( normProj == 0.f )
		return 0;	
...
{	
	...
	{		
		...
		// if ray is in VO		
		// эту штуку тоже можно кешировать
		// проверяем, находится ли точка 0,0 внутри конуса
