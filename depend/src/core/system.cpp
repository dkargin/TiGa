...
LRESULT CALLBACK GuestWindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam);
...
bool CALL HGE_Impl::System_Initiate()
{
	...
	if( !bGuest )
	{
		...
		styleW=WS_POPUP|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX|WS_VISIBLE|WS_OVERLAPPED; //WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX;
		...
		if(hwndParent)
		{
			...
			GetClientRect(hwndParent,&rectW);			
	...
	if(hwndParent && bGuest)
	{
		oldWndProc = (WNDPROC)GetWindowLong(hwndParent,GWL_WNDPROC);
		SetWindowLong(hwndParent, GWL_WNDPROC, (LONG)GuestWindowProc);
		RECT rc;
...
bool CALL HGE_Impl::System_Start()
{
	...
	// there were some problems for guest mode, when some of the events were not handled correctly
	// i.e INPUT_MBUTTONUP event after INPUT_MBUTTONDOWN
	if(!bGuest)
		_ClearQueue();
...
void CALL HGE_Impl::System_SetStateBool(hgeBoolState state, bool value)
{
	switch(state)
	{
		...
		#endif
		case HGE_GUEST: this->bGuest = value; break;
	}
}
...
int CALL HGE_Impl::System_GetStateInt(hgeIntState state)
{
	switch(state)
	{
		case HGE_SCREENWIDTH:	
			...
			{
				RECT rc;
				GetWindowRect(bGuest?hwndParent:hwnd,&rc);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	{	
		...
		case WM_PAINT:
			if(pHGE->_GfxStarted() && pHGE->procRenderFunc && pHGE->bWindowed) pHGE->procRenderFunc();
			break;
		...
		case WM_ACTIVATE:
			...
			if(pHGE->_GfxStarted() && pHGE->bActive != bActivating) pHGE->_FocusChange(bActivating);
		...
		case WM_SIZE:
			if(pHGE->_GfxStarted() && wparam==SIZE_RESTORED) pHGE->_Resize(LOWORD(lparam), HIWORD(lparam));
			//return FALSE;
			break;
...
LRESULT CALLBACK GuestWindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	{		
		case WM_PAINT:
			if(pHGE->_GfxStarted() && pHGE->procRenderFunc && pHGE->bWindowed) 
				pHGE->procRenderFunc();
			//return FALSE; 
			break;
	...
	return CallWindowProc(pHGE->oldWndProc,hwnd, msg, wparam, lparam);
}
